...
**Option A — Native install (Linux/macOS/WSL with apt):**
```bash
pip install manim==0.20.1
sudo apt-get install texlive-latex-base texlive-latex-extra texlive-fonts-recommended ffmpeg
```

**Option B — Docker (no sudo, WSL, macOS without Homebrew):**
See `references/docker-workspace.md` for a working `Dockerfile`. Manim's C-extension deps (`manimpango → pangocairo → libpango1.0-dev + build-essential`) are fragile outside Docker — the docker path is more reliable on WSL and headless servers.
...
3. **RENDER** — `manim -ql script.py Scene1 Scene2 ...` for draft, `manim -qh --format mp4 script.py Scene1 Scene2 ...` for production
...
manim -qh --format mp4 script.py Scene1_Introduction Scene2_CoreConcept  # production (outputs to media/ by default)
```

### Step 4: Stitch

```bash
# Find rendered files (manim outputs to media/videos/script/<quality>/)
cat > concat.txt << 'EOF'
...
| `references/docker-workspace.md` | Docker rendering workflow, pangocairo dep, bookworm apt sources, concat/mux |
| `references/bookshelf-to-manim.md` | Picking bookSHelf sections for video, topic selection criteria, output structure |
